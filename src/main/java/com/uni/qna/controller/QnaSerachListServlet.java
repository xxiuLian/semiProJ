...
/**
 * Servlet implementation class QnaSerachListServlet
 */
@WebServlet("/searchQna.do")
public class QnaSerachListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public QnaSerachListServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//총 게시글 개수
		listCount = new QnaService().getSearchListCount(keyword);
		...
		ArrayList<Qna> list = new QnaService().selectSearchList(pi, keyword);
		request.setAttribute("keyword", keyword);
		request.setAttribute("list", list);
		request.setAttribute("pi", pi);
		request.getRequestDispatcher("views/qna/qnaSearchListView.jsp").forward(request, response);	
	}
