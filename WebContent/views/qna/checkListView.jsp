		...
		--%>
				...
				<div class="pagingArea" align="center">
					...
					<button class="pagingicon"
						onclick="location.href='${contextPath}/checkQnaList.do?currentPage=1'">
						&lt;&lt;</button>
					...
					<c:choose>
						...
						<c:otherwise>
							<button class="pagingicon"
								onclick="location.href='${contextPath}/checkQnaList.do?currentPage=${currentPage - 1}'">
								&lt;</button>
						</c:otherwise>
					</c:choose>
					<!-- 페이지 목록 -->
					<c:forEach var="p" begin="${startPage}" end="${endPage}" step="1">
						<c:choose>
							...
							<c:otherwise>
								<button class="pagingicon"
									onclick="location.href='${contextPath}/checkQnaList.do?currentPage=${p}'">
									${p}</button>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					...
					<!-- 맨 끝으로 (>>) -->
					<button class="pagingicon"
						onclick="location.href='${contextPath}/checkQnaList.do?currentPage=${maxPage}'">
						&gt;&gt;</button>
				</div>
	...
	if(!${empty list}){
		$(function(){
			$("#tbody>tr").click(function(){
				...
				location.href = "${contextPath}/checkDetailQna.do?qno="+qno;
			})
		})
	}
